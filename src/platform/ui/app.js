(() => {
  ...
  const GAME_PRESENTATION_OVERRIDES = {
    "arena_live": {
      ...
      previewImage: ARENA_PREVIEW_IMAGE,
      description: "Arena competitiva conectada al live con powers y ranking en tiempo real.",
      meta: [],
    },
  };
  ...
  function renderAdvancedLogs() {
    ...
    els.advancedLogOutput.textContent = [...state.terminalLines, ...runnerLines, ...gameLines].join("\n");
  }
  ...
  function latencyPresentation(value, connected) {
    ...
    if (!connected || !Number.isFinite(latencyMs) || latencyMs > 600000) {
      return { text: "Sin se\u00f1al", tone: "danger" };
    }
    if (latencyMs < 1000) {
      return { text: `${Math.round(latencyMs)} ms`, tone: "live" };
    }
    if (latencyMs < 5000) {
      return { text: `${(latencyMs / 1000).toFixed(1)} s`, tone: "warn" };
    }
    return { text: `${Math.round(latencyMs / 1000)} s`, tone: "danger" };
  }
  ...
  function renderGames(payload) {
    ...
    if (!items.length) {
      els.gamesList.innerHTML = `<div class="empty-state">No hay juegos disponibles ahora mismo.</div>`;
      return;
    }

    els.gamesList.innerHTML = items.map((item) => {
  ...
  function renderRecentActivity(payload) {
    ...
    if (!items.length) {
      ...
      els.recentActivityList.innerHTML = `<div class="empty-state">${escapeHtml(emptyText)}</div>`;
      return;
    }
  ...
  async function loadRealtime(force = false) {
    ...
    try {
      const [eventsPayload, metricsPayload] = await Promise.all([
        apiGetJson("/api/events"),
        apiGetJson("/api/metrics"),
      ]);
  ...
  function init() {
    ...
    updateTemplateSummary();
    refreshAll(true);
    window.setInterval(() => loadState(false), POLL_STATE_MS);
    window.setInterval(() => loadRealtime(false), POLL_REALTIME_MS);
